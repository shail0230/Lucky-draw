...
export default function LuckyDraw() {
  ...
  function updatePhone(value: string, complete = false) {
    const cleaned = value.replace(/[\s()-]/g, "").replace(/^\+91/, "");
    setPhone(cleaned); setError("");
    if (cleaned && (!/^[6-9]\d*$/.test(cleaned) || cleaned.length > 10 || (complete && cleaned.length !== 10))) {
      setError("Enter 10 digits starting with 6, 7, 8 or 9.");
    }
  }
  ...
  return <div className="site-shell">
    <header className="site-header">
      <a href="/" className="brand" aria-label="Madras Pesu Lucky Draw home"><img className="brand-logo" src="/madras-pesu-logo.jpeg" alt="Madras Pesu - Full meals, full talks" width={1600} height={1600} /></a>
    ...
    <main className="main-layout">
      <section className="story-panel" aria-label="Madras Pesu lucky draw campaign">
        ...
        <div className="prize-preview"><p className="eyebrow">A taste of what you could win</p><ul><li><strong>1 year</strong><span>of bene dosa</span></li><li><strong>Free dosa</strong><span>A treat on us</span></li><li><strong>10-20% off</strong><span>Your next bill</span></li></ul><p>Sample prizes for this demo. Your offer is revealed after verification.</p></div>
      ...
      <section className="entry-panel" aria-label="Enter the lucky draw">
        ...
        <div className="form-card">
          {restoring ? <div className="loading-state" role="status"><LoaderCircle className="animate-spin" /><p>Preparing a little good fortune…</p></div> : <div key={step} className="step-content">
            ...
            <p className="form-description">{step === 0 ? "A few seconds. A little scratch. A delicious surprise." : step === 1 ? `Enter the 6-digit code for ${maskedPhone}.` : coupon?.revealed ? "Keep this coupon handy for your next visit." : "Your offer is waiting under the lime. Go on, give it a scratch."}</p>
            {notice && <p className="notice" role="status">{notice}</p>}
            {step === 0 && <form onSubmit={sendOtp} noValidate>
              <label htmlFor="phone">Mobile number</label>
              <div className={`phone-field ${error ? "invalid" : ""}`}><span className="country-code">+91</span><input id="phone" type="tel" inputMode="tel" autoComplete="tel-national" placeholder="Enter 10-digit number" value={phone} onChange={e => updatePhone(e.target.value)} onBlur={() => updatePhone(phone, true)} maxLength={24} aria-invalid={!!error} aria-describedby={error ? "form-error" : "phone-hint"} disabled={busy} /></div>
              ...
              <button className="primary-button" type="submit" disabled={busy}>{busy ? <><LoaderCircle className="animate-spin" size={18} /> Sending code…</> : <>Send OTP <ArrowRight size={19} /></>}</button>
            </form>}
            {step === 1 && <form onSubmit={verify} noValidate>
              ...
              {error && <p id="form-error" className="error-message" role="alert">{error}</p>}
              <button className="primary-button" type="submit" disabled={busy || expired}>{busy ? <><LoaderCircle className="animate-spin" size={18} /> Please wait…</> : <>Verify &amp; unlock offer <ArrowRight size={19} /></>}</button>
        ...
        <details className="redemption-help"><summary>How do I use my offer?</summary><ol><li>Verify your number and reveal your coupon.</li><li>Save the coupon image or copy its code.</li><li>Show the code to the Madras Pesu restaurant team before billing.</li></ol><p>One offer per mobile number. Valid until {CAMPAIGN.validUntil}. This is a demo with sample prizes; restaurant redemption and prize-specific conditions must be confirmed before a live launch.</p></details>
    ...
    <footer className="site-footer"><span>With love, from Madras Pesu.</span><span>Demo experience · Sample prizes · No SMS sent</span></footer>
  </div>;
}
